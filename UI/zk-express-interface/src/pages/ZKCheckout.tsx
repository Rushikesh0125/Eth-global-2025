...
const ZKCheckout: React.FC = () => {
  const [uniqueSelfId, setUniqueSelfIf] = useState<IdentityNullifier>({
    state: "idle",
    value: null,
  });

  const isInitialSelfIdState = uniqueSelfId.state === "idle";
  const isFetchingSelfIdState = uniqueSelfId.state === "fetching";
  const isfetchedSelfIdState = uniqueSelfId.state === "fetching";

  useEffect(() => {
    if (uniqueSelfId.state === "fetching") {
      // fetch self id
      const selfId = localStorage.getItem("selfId");
    }
  }, [uniqueSelfId]);

  const { selfApp } = useSelfIdentity({
    disclosures: {
      // configstore (see separate docs)
      minimumAge: 18,
      excludedCountries: [],
      ofac: true,
    },
  ...
  const [walletAddress, setWalletAddress] = useQueryState(
    "wallet",
    parseAsString
  );
  ...
  const [isProcessing, setIsProcessing] = useQueryState(
    "processing",
    parseAsBoolean
  );
  const [selfVerified, setSelfVerified] = useQueryState(
    "selfVerified",
    parseAsBoolean
  );

  const handleSelfVerify = () => {
    setUniqueSelfIf({ state: "fetching", value: null });
  };
                    ...
                    <div className="flex items-center gap-3 mt-2">
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <span className="text-xs text-gray-300">
                          {mockMerchant.reputation.rating}
                        </span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-4">
                    <div className="p-4 bg-purple-500/10 rounded-lg border border-purple-500/20">
                      ...
                      <p className="text-sm text-gray-300">
                        Verify your identity using Self protocol for enhanced
                        security and reputation benefits.
                      </p>
                    </div>

                    {isInitialSelfIdState ? (
                      <Button
                      ...
                      <div>Some Error. Please try again.</div>
                    )}
                  </div>
                ) : isfetchedSelfIdState ? (
